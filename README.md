...
# Introduction
Micro-service application with Eureka, Config Server, Zuul, Proxy based on DiscoveryClient Bean, Kafka(producer-consumer), and Rest Crud.
You have to run a kafka on localhost:9092 before launching this instances, since the microservices rely on config server and eureka. The order
of launch should be:
1. Eureka
1. Config Server
1. Todo Service
...
The only requirement are the first two, the others you can launch them in any order, but I strongly recommend you to launch
Gateway service the last, otherwise you may have to wait Zuul to fetch from Eureka the next time and hopefully the
other microservices he routes through are now available.
